...
```{r}
# Violin plots
Vdiff_plot<- cbind.data.frame(BF01_detV$samples, BF01_cohV$samples,BF01_helpV$samples)
colnames(Vdiff_plot)<- c("Detail", "Coherence", "Willlingness to help")
Vdiff_plot<- Vdiff_plot %>% gather("Detail", "Coherence", "Willlingness to help", key= Condition, value= Posterior)
violin_Vdiff<- violin_bayes(Vdiff_plot)[[1]]
violin_Vdiff +
  labs(x="Measures", y = "Version difference")+
