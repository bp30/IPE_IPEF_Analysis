...
setwd('C:/Users/dpen466/Google Drive/Phd (1)/PhD_writing/IPE_IPEF/Results/Analysis/') #to change

# Source custom functions
source("source/LMM_scripts.R")
source("source/Bayes_scripts.R")
```
...
Vdiff_det<- lmer(Detail.response~Version+(1|Participant)+(1|Story_ID), data=Exp1.df)
saveRDS(Vdiff_det, "Vdiff_det.rds")
summary(readRDS("Vdiff_det.rds"))
#-----------------------------------------------------------------------------------------------------------------------
# Coherence--Model:lmer(Coherence.response~Version+(1|Participant)+(1|Story_ID), data=Exp1.df)
Vdiff_coh<- lmer(Coherence.response~Version+(1|Participant)+(1|Story_ID), data=Exp1.df)
saveRDS(Vdiff_coh, "Vdiff_coh.rds")
summary(readRDS("Vdiff_coh.rds"))
#-----------------------------------------------------------------------------------------------------------------------
# Willingness to help--Model:lmer(help.response~Version+(1|Participant)+(1|Story_ID), data=Exp1.df)
Vdiff_help<-lmer(help.response~Version+(1|Participant)+(1|Story_ID), data=Exp1.df)
saveRDS(Vdiff_help, "Vdiff_help.rds")
...
```{r}
ranova(lmer(help.response~(1|Participant)+(1|Story_ID), data=Exp1.df))
# Both varying effect structures were significant and thus are included for subsequent analyses 
```
### Replication of prosocial simulation effect
```{r}
# Parsimonious model: lmer(help.response~Condition+(dummy_img+dummy_est|Participant)+(1|Story_ID), data= Exp1.df)
step(lmer(help.response~Condition+(dummy_est+dummy_img||Participant)+(dummy_est+dummy_img||Story_ID), data= Exp1.df))
zcp_rm1<- lmer(help.response~Condition+(dummy_img+dummy_est||Participant)+(1|Story_ID), data= Exp1.df)
PSE_exp1<- lmer(help.response~Condition+(dummy_img+dummy_est|Participant)+(1|Story_ID), data= Exp1.df)
anova(zcp_rm1,PSE_exp1, refit=F)
saveRDS(PSE_exp1, "PSE_exp1.rds")

# Summary
summary(PSE_exp1<-readRDS("PSE_exp1.rds"))
# F-test with Kenward-Roger approximation of degress of freedom
anova(PSE_exp1, test='kenward-roger')
# Effect Size
...
step(lmer (help.response~Condition*Detail_C+(dummy_img*Detail_C||Participant)+ (dummy_img*Detail_C||Story_ID),data= Exp1_int))
int_rm_det1<- lmer (help.response~Condition*Detail_C+(dummy_img+dummy_img:Detail_C||Participant)+(Detail_C||Story_ID),data= Exp1_int)
int_det_exp1<- lmer (help.response~Condition*Detail_C+(dummy_img+dummy_img:Detail_C|Participant)+(Detail_C|Story_ID),data= Exp1_int)
int_det_exp1<-refit_LMM(int_det_exp1) ## Model failed to converge, refit the model with better starting values (i.e. end values of previous fits) for REML. Model converged after 3 refits
anova(int_rm_det1,int_det_exp1, refit=F) ## Inlcuding correlation parameters significantly improve model fit.
saveRDS(int_det_exp1, "int_det_exp1.rds")

# Summary
summary(int_det_exp1<-readRDS("int_det_exp1.rds"))
confint(int_det_exp1, "ConditionImagine:Detail_C")
# F-test with Kenward-Roger approximation of degress of freedom
anova(int_det_exp1, test='kenward-roger')
# Effect Size
...
step(lmer (help.response~Condition*Coherence_C+(dummy_img*Coherence_C||Participant)+(dummy_img*Coherence_C||Story_ID),data= Exp1_int)) 
int_rm_coh1<- lmer (help.response~Condition*Coherence_C+(dummy_img||Participant)+ (Coherence_C||Story_ID),data= Exp1_int)
int_coh_exp1<- lmer (help.response~Condition*Coherence_C+(dummy_img|Participant)+(Coherence_C|Story_ID),data= Exp1_int)
int_coh_exp1<-refit_LMM(int_coh_exp1)## Model converged after 3 refits
anova(int_rm_coh1,int_coh_exp1, refit=F)
saveRDS(int_coh_exp1, "int_coh_exp1.rds")

# Summary
summary(int_coh_exp1<-readRDS("int_coh_exp1.rds"))
confint(int_coh_exp1, "ConditionImagine:Coherence_C")
# F-test with Kenward-Roger approximation of degress of freedom
anova(int_coh_exp1, test='kenward-roger')
# Effect Size
...
# Detail
library (ordinal)
oridnal1.df<- Exp1_int
oridnal1.df$help.response<- as.factor (oridnal1.df$help.response)
ordinal_det1<-clmm(help.response~Condition*Detail_C+(dummy_img+dummy_img:Detail_C|Participant)+(Detail_C|Story_ID),data= oridnal1.df,link="logit")
saveRDS(ordinal_det1, "ordinal_det1.rds")
...
```{r}
# Detail
ggpredict(int_det_exp1, c("Detail_C[-4,0,5]","Condition"))%>% plot() + 
...
ggpredict(int_coh_exp1, c("Coherence_C[-4,0,4]","Condition"))%>% plot() + 
  ...
  scale_fill_discrete(labels = c("Estimate Helping", "Imagine Helping"))
```
## Bayesian Cumulative Link Mixed Models
### Assess differences between versions
```{r}
# Detail-- model: 
Vdiff_det_bayes<- brm(data= Exp1_int, family=cumulative("logit"), 
                  bf(Detail.response~Version+(1|Participant)+(Version|Story_ID)) +
...
```{r}
# Model
Bayes_Help_exp1<- brm(data= Exp1.df, family=cumulative("logit"), 
                  ...
                  prior= c(prior(normal (0.2,1), class=b, coef='ConditionImagine'),    
                           ...
                           prior(lkj(1), class = cor)), 
                  iter=7000,warmup=2000,chains=8, cores=8, seed=67, sample_prior=T, save_all_pars = T)
saveRDS(Bayes_Help_exp1, "Bayes_Help_exp1.rds")
...
** 2) ordered model1: Imagine > Estimate > Identify
** 3) ordered model2: Imagine > Estimate = Identify

```{r}
# Null model 
null_exp1<- brm(data= Exp1.df, family=cumulative("logit"), 
                  help.response~1+(1|Participant)+(1|Story_ID),
                  prior= c(prior(normal(0,1.2), class=Intercept),
                           prior(exponential(1), class= sd)), 
                  iter=7000,warmup=2000,chains=8, cores=8, seed=67, sample_prior=T, save_all_pars = T)

# The first order restricted model: Imagine > Estimate > Identify
## First, we compare the unrestricted model to the null model using the brms::bayes_factor function 
set.seed (67)
unrestricted_exp1 <- bayes_factor(Bayes_Help_exp1, null_exp1)
unrestricted.vsnull_exp1 <- unrestricted_exp1$bf

## Then, we can compare the unrestricted model to the first order restricted model
restrictedmodel1_exp1<- (posterior_exp1[, "b_ConditionEstimate"] > 0) & 
  (posterior_exp1[, "b_ConditionImagine"] > 0) &
  (posterior_exp1[, "b_ConditionImagine"]>posterior_exp1[, "b_ConditionEstimate"])
(restrictedmodel1_BF_exp1<-restricted_BF(restrictedmodel1_exp1, 4e4, 3))

##Then, we can compare the null to the order restricted model, by transitive property:
(restricted1.vsnull_exp1 <- restrictedmodel1_BF_exp1 * unrestricted.vsnull_exp1)

# The second order restricted model: Imagine > Estimate = Identify
## Then, we can compare the unrestricted model to the second order restricted model
restrictedmodel2_exp1<- (abs(posterior_exp1[, "b_ConditionEstimate"]) < 0.05) & 
  (posterior_exp1[, "b_ConditionImagine"] >0) &
  (posterior_exp1[, "b_ConditionImagine"]>posterior_exp1[, "b_ConditionEstimate"])
(restrictedmodel2_BF_exp1<-restricted_BF(restrictedmodel2_exp1, 4e4, 3))

## By transitive property:
(restricted2.vsnull_exp1 <- restrictedmodel2_BF_exp1 * unrestricted.vsnull_exp1)

# Contrast the 2 order restricted models
(restricted1.vsrestricted2_exp1 <- restrictedmodel1_BF_exp1 * (1/(restrictedmodel2_BF_exp1)))
```
...
```{r}
# Detail -- Model:
Bayes_det_exp1<- brm(data= Exp1_int, family=cumulative("logit"), 
                  ...
                  prior= c(prior(normal (0.2,1), class=b), 
                           ...
                           prior(lkj(1), class = cor)),
                  iter=7000,warmup=2000,chains=8, cores=8, seed=67, sample_prior=T, save_all_pars = T)
saveRDS(Bayes_det_exp1, "Bayes_det_exp1.rds")
...
#------------------------------------------------------------------------------------------------------------------------
# Coherence -- Model: 
Bayes_coh_exp1<- brm(data= Exp1_int, family=cumulative("logit"), 
...
saveRDS(Bayes_coh_exp1, "Bayes_coh_exp1.rds")
...
# Summary
summary(Bayes_coh_exp1, prob=0.95)
# pd
...
# Summary
(posterior_summary(det_posterior1, probs=  c(0.055, 0.945)))
# pd 
...
# Summary
(posterior_summary(coh_posterior1, probs=  c(0.055, 0.945)))
# pd 
...
```{r}
ranova(lmer(help.response~(1|participant)+(1|Story_ID)+(1|Faces_ID), data=Exp2.df))
# Both participant and Story_ID varying effects significantly improve model fit so should included. However, face ID was not significant and thus was excluded from future models. 
```

### Replication of Prosoical simulation effect
```{r}
# Parsimonious model: lmer(help.response~Condition+Gender_C*Face_gender_C+(dummy_img|participant)+(1|Story_ID), data= Exp2.df)
step(lmer(help.response~Condition+Gender_C*Face_gender_C+(dummy_est+dummy_img||participant)+(dummy_est+dummy_img||Story_ID), data= Exp2.df)) 
zcp_rm2<- lmer(help.response~Condition+Gender_C*Face_gender_C+(dummy_img||participant)+(1|Story_ID), data= Exp2.df)
PSE_exp2<- lmer(help.response~Condition+Gender_C*Face_gender_C+(dummy_img|participant)+(1|Story_ID), data= Exp2.df)
PSE_exp2<-refit_LMM(PSE_exp2) # 1 refit
anova(zcp_rm2,PSE_exp2, refit=F)
saveRDS(PSE_exp2, "PSE_exp2.rds")

# Summary
summary(PSE_exp2<-readRDS("PSE_exp2.rds"))
# F-test with Kenward-Roger approximation of degress of freedom
anova(PSE_exp2, test='kenward-roger')
# Effect Size
...
step(lmer (help.response~Condition*Detail_C+Gender_C*Face_gender_C+(dummy_img*Detail_C||participant)+ (dummy_img*Detail_C||Story_ID),data= Exp2_int)) 
int_rm_det2<- lmer (help.response~Condition*Detail_C+Gender_C*Face_gender_C+(dummy_img+Detail_C||participant)+(1|Story_ID),data= Exp2_int)
int_rm_det2<- refit_LMM(int_rm_det2)
int_det_exp2<- lmer (help.response~Condition*Detail_C+Gender_C*Face_gender_C+(dummy_img+Detail_C|participant)+(1|Story_ID),data= Exp2_int)
int_det_exp2<- refit_LMM(int_det_exp2)
anova(int_rm_det2,int_det_exp2, refit=F)
saveRDS(int_det_exp2, "int_det_exp2.rds")

# Summary
summary(int_det_exp2<-readRDS("int_det_exp2.rds"))
confint(int_det_exp2, "ConditionImagine:Detail_C")
# F-test with Kenward-Roger approximation of degress of freedom
anova(int_det_exp2, test='kenward-roger')
# Effect Size
...
step(lmer (help.response~Condition*Coherence_C+Gender_C*Face_gender_C+(dummy_img*Coherence_C||participant)+(dummy_img*Coherence_C||Story_ID),data= Exp2_int))
int_rm_coh2<- lmer (help.response~Condition*Coherence_C+Gender_C*Face_gender_C+(dummy_img+Coherence_C||participant)+ (1|Story_ID),data= Exp2_int)
int_rm_coh2<- refit_LMM(int_rm_coh2) # one refit
int_coh_exp2<- lmer (help.response~Condition*Coherence_C+Gender_C*Face_gender_C+(dummy_img+Coherence_C|participant)+(1|Story_ID),data= Exp2_int)
anova(int_rm_coh2,int_coh_exp2, refit=F)
saveRDS(int_coh_exp2, "int_coh_exp2.rds")

# Summary
summary(int_coh_exp2<-readRDS("int_coh_exp2.rds"))
confint(int_coh_exp2, "ConditionImagine:Coherence_C")
# F-test with Kenward-Roger approximation of degress of freedom
anova(int_coh_exp2, test='kenward-roger')
# Effect Size
...
# Detail
library (ordinal)
oridnal2.df<- Exp2_int
oridnal2.df$help.response<- as.factor (oridnal2.df$help.response)
ordinal_det2<-clmm(help.response~Condition*Detail_C+Gender_C*Face_gender_C+(dummy_img+Detail_C|participant)+(1|Story_ID),data=oridnal2.df,link="logit")
saveRDS(ordinal_det2, "ordinal_det2.rds")
summary(ordinal_det2)
```
#### Plots
```{r}
# Detail
ggpredict(int_det_exp2, c("Detail_C[-4,0,5]","Condition"))%>% plot() + 
  labs(title="",x="Detail", y = "Willingness to help") + 
  theme_classic() +
  guides(fill=guide_legend(title="Simulation Condition")) +  
  scale_fill_discrete(labels = c("Estimate Helping", "Imagine Helping"))
...
## Bayes cumulative link mixed models
...
rope(ER_help1$samples,ci=1, range = c(-0.1, 0.1))
```
### Prosocial simulation effect
```{r}
# Model
Bayes_Help_exp2<- brm(data= Exp2.df, family=cumulative("logit"), 
                  ...
                  prior= c(prior(normal (0.2, 1), class=b, coef='ConditionImagine'),  
                           ...
                           prior(lkj(1), class = cor)), 
                  iter=7000,warmup=2000,chains=8, cores=8, seed=67, sample_prior=T, save_all_pars = T)
saveRDS(Bayes_Help_exp2, "Bayes_Help_exp2.rds")

Bayes_Help_exp2<- readRDS("Bayes_Help_exp2.rds")
# Diagnostic Checks: pp check and traceplots
pp_check(Bayes_Help_exp2)
plot(Bayes_Help_exp2)
# Total number of Sample size and sample sizes for each parameter
...
```{r}
# Null model 
null_exp2<- brm(data= Exp2.df, family=cumulative("logit"), 
                  help.response~1+(1|participant)+(1|Story_ID)+ (1|Faces_ID),
                  prior= c(prior(normal(0,1.2), class=Intercept),
                           prior(exponential(1), class= sd)), 
                  iter=7000,warmup=2000,chains=8, cores=8, seed=67, sample_prior=T, save_all_pars = T)
...
## First, we compare the unrestricted model to the null model using the brms::bayes_factor function 
set.seed (67)
unrestricted_exp2 <- bayes_factor(Bayes_Help_exp2, null_exp2)
(unrestricted.vsnull_exp2 <- unrestricted_exp2$bf)

## Then, we can compare the unrestricted model to the first order restricted model
restrictedmodel1_exp2<- (posterior_exp2[, "b_ConditionEstimate"] > 0) & 
  (posterior_exp2[, "b_ConditionImagine"] > 0) &
  (posterior_exp2[, "b_ConditionImagine"]>posterior_exp2[, "b_ConditionEstimate"])
(restrictedmodel1_BF_exp2<-restricted_BF(restrictedmodel1_exp2, 4e4, 3))

##Then, we can compare the null to the order restricted model, by transitive property:
(restricted1.vsnull_exp2 <- restrictedmodel1_BF_exp2 * unrestricted.vsnull_exp2)

# The second order restricted model: Imagine > Estimate = Identify
## Then, we can compare the unrestricted model to the second order restricted model
restrictedmodel2_exp2<- (abs(posterior_exp2[, "b_ConditionEstimate"]) < 0.05) & 
  (posterior_exp2[, "b_ConditionImagine"] >0) &
  (posterior_exp2[, "b_ConditionImagine"]>posterior_exp2[, "b_ConditionEstimate"])
(restrictedmodel2_BF_exp2<-restricted_BF(restrictedmodel2_exp2, 4e4, 3))

## By transitive property:
(restricted2.vsnull_exp2 <- restrictedmodel2_BF_exp2 * unrestricted.vsnull_exp2)

# Contrast the 2 order restricted models
(restricted1.vsrestricted2_exp2 <- restrictedmodel1_BF_exp2 * (1/(restrictedmodel2_BF_exp2)))
```
...
```{r}
# Detail -- Model:
Bayes_det_exp2<- brm(data= Exp2_int, family=cumulative("logit"), 
                  ...
                  prior= c(prior(normal (0.2,1), class=b), 
                           ...
                           prior(lkj(1), class = cor)),
                  iter=7000,warmup=2000,chains=8, cores=8, seed=67, sample_prior=T, save_all_pars = T)
saveRDS(Bayes_det_exp2, "Bayes_det_exp2.rds")
...
#-------------------------------------------------------------------------------------------------------------------------------------------------------
# Coherence -- Model: 
Bayes_coh_exp2<- brm(data= Exp2_int, family=cumulative("logit"), 
                  bf(help.response~Condition*Coherence_C+(Condition*Coherence_C|participant)+(Condition*Coherence_C|Story_ID) +
                       (Condition*Coherence_C|Faces_ID)) +
                    lf(disc ~ 0+Condition, cmc=F),
                  prior= c(prior(normal (0.2,1), class=b),
                           prior(normal(0,1.2), class=Intercept),
                           prior(exponential(1), class= sd),
                           prior(lkj(1), class = cor)),
                  iter=7000,warmup=2000,chains=8, cores=8, seed=67,  sample_prior=T, save_all_pars = T)
saveRDS(Bayes_coh_exp2, "Bayes_coh_exp2.rds")
...
# Summary
(posterior_summary(det_posterior2, probs=  c(0.055, 0.945)))
# pd 
...
# Summary
(posterior_summary(coh_posterior2, probs=  c(0.055, 0.945)))
# pd 
...
## Bayesian CLMMs on combined datasets of both experiments
### Data set up 
```{r}
...
```{r}
# Detail
step(lmer (help.response~Condition*Detail_C+(dummy_img*Detail_C||Participant)+ (dummy_img*Detail_C||Story_ID),data= combined.df))
int_rm_det3<- lmer (help.response~Condition*Detail_C+(dummy_img+Detail_C|Participant)+(Detail_C||Story_ID),data= combined.df)
int_rm_det3<- refit_LMM(int_rm_det3)
int_det_exp3<- lmer (help.response~Condition*Detail_C+(dummy_img+dummy_img:Detail_C|Participant)+(Detail_C|Story_ID),data= combined.df)

## Test of fixed effects using F-test
anova(int_det_exp3, test='kenward-roger')
summary(int_det_exp3);confint(int_det_exp3, "ConditionImagine:Detail_C")
#Detail (not significant)
ggpredict(int_det_exp3, c("Detail_C[-4,0,4]","Condition"))%>% plot()+ labs(title="",x="Detail", y = "Willingness to help")+theme_classic()

# frequentist ordinal analysis
library (ordinal)
test<- combined.df
test$help.response<- as.factor (test$help.response)
summary(clmm(help.response~Condition*Detail_C+(dummy_img+dummy_img:Detail_C|Participant)+(Detail_C|Story_ID),data= test,link="logit"))
```
### Bayesian CLMMs
#### 3-way interactions
...
```{r}
# Detail--Model:
Bayes_det_3way<- brm(data= combined.df, family=cumulative("logit"), 
                  ...
                  prior= c(prior(normal (0,1), class=b), 
                           ...
                           prior(lkj(1), class = cor)),
                  iter=7000,warmup=2000,chains=8, cores=8, seed=67, sample_prior=T, save_all_pars = T)
saveRDS(Bayes_det_3way, "Bayes_det_3way.rds")
...
#--------------------------------------------------------------------------------------------------------------------------------------------------------
# Coherence--Model:
Bayes_coh_3way<- brm(data= combined.df, family=cumulative("logit"), 
                  ...
                  prior= c(prior(normal (0,1), class=b), 
                           ...
                           prior(lkj(1), class = cor)),
                  iter=7000,warmup=2000,chains=8, cores=8, seed=67, sample_prior=T, save_all_pars = T)
saveRDS(Bayes_coh_3way, "Bayes_coh_3way.rds")
...
# BF01
(BF01_det_3way<-hypothesis(Bayes_coh_3way, hypothesis ="ConditionImagine:Coherence_C:Experiment2 =0", seed=67, alpha=0.055));plot(BF01_det_3way)
# pd
colMeans(BF01_det_3way$samples>0/4e4)
# % in ROPE
rope(BF01_det_3way$samples,ci=1, range = c(-0.1, 0.1))
...
```{r}
# Detail--Model:
Bayes_det_combine<- brm(data= combined.df, family=cumulative("logit"), 
                  bf(help.response~Condition*Detail_C+(Condition*Detail_C|Participant)+(Condition*Detail_C|Story_ID))+
                       lf(disc ~ 0+Condition, cmc=F), 
                  prior= c(prior(normal (0.2,1), class=b), 
                           prior(normal(0,1.2), class=Intercept),
                           ...
                           prior(lkj(1), class = cor)),
                  iter=7000,warmup=2000,chains=8, cores=8, seed=67, sample_prior=T, save_all_pars = T)
saveRDS(Bayes_det_combine, "Bayes_det_combine.rds")
...
#-------------------------------------------------------------------------------------------------------------------------------------------------------
# Coherence--Model:
Bayes_coh_combine<- brm(data= combined.df, family=cumulative("logit"), 
                  bf(help.response~Condition*Coherence_C+(Condition*Coherence_C|Participant)+(Condition*Coherence_C|Story_ID))+
                       lf(disc ~ 0+Condition, cmc=F), 
                  prior= c(prior(normal (0.2,1), class=b), 
                           prior(normal(0,1.2), class=Intercept),
                           ...
                           prior(lkj(1), class = cor)),
                  iter=7000,warmup=2000,chains=8, cores=8, seed=67, sample_prior=T, save_all_pars = T)
saveRDS(Bayes_coh_combine, "Bayes_coh_combine.rds")
...
```{r}
# Violin plots -- Detail: separate conditions
det_posterior3<- cbind.data.frame("Estimate Helping" = posterior_det_combine$b_Detail_C, "Imagine Helping" = posterior_det_combine$b_Detail_C + posterior_det_combine$`b_ConditionImagine:Detail_C`)
plot_det3<- det_posterior2 %>% gather("Estimate Helping", "Imagine Helping", key= Condition, value= Posterior)
violin_det3<- violin_bayes(plot_det3, rope= c(-0.1, 0.1))[[1]]
violin_det3+
  labs(x="Condition", y = expression (beta))+
  stat_boxplot(geom = "errorbar", width = 0.1, coef=0.7)+
  theme(legend.position='none')+
  ylim(-0.2, 1.5)

# Summary
(posterior_summary(det_posterior3, probs=  c(0.055, 0.945)))
# pd 
p_direction(det_posterior3)
# % in ROPE
rope(det_posterior3,ci=1, range = c(-0.10, 0.10))
#-------------------------------------------------------------------------------------------------------------------------------------------------------
# Violin plots -- Coherence: Separate Conditions
coh_posterior3<- cbind.data.frame('Estimate Helping' = posterior_coh_combine$b_Coherence_C, "Imagine Helping" = posterior_coh_combine$b_Coherence_C + posterior_coh_combine$`b_ConditionImagine:Coherence_C`)
plot_coh3<- coh_posterior3 %>% gather("Estimate Helping", "Imagine Helping", key= Condition, value= Posterior)
violin_coh3<- violin_bayes(plot_coh3, rope=c(-0.1, 0.1))[[1]]
violin_coh3+
  labs(x="Simulation Condition", y = expression (beta))+
  stat_boxplot(geom = "errorbar", width = 0.1, coef=0.7)+
  theme(legend.position='none') +
...
# Summary
(posterior_summary(coh_posterior3, probs=  c(0.055, 0.945)))
# pd 
p_direction(coh_posterior3)
# % in ROPE
rope(coh_posterior3,ci=1, range = c(-0.10, 0.10))
#-------------------------------------------------------------------------------------------------------------------------------------------------------
# Violin plots -- Detail and Coherence interaction effects
